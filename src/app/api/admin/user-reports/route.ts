...
export async function GET(request: Request) {
  try {
    ...
    // Check cache first
    const cacheKey = `user_reports_${status}_${page}_${limit}_${roomId || 'all'}`;
    const cached = cache.get(cacheKey);
    if (cached) {
      return NextResponse.json({ ...cached, fromCache: true });
    }

    let query: any = adminDb.collection('userReports');
...
export async function PATCH(request: Request) {
  try {
    ...
    const reportRef = adminDb.collection('userReports').doc(reportId);
    ...
    if (status === 'resolved' && reportData?.reportedUserId) {
      ...
      // 1. Warn the user
      // 2. Temporarily ban the user from the room
      // 3. Send notification to reported user
      // 4. Increment user's violation count
      
      // For now, just log it
      console.log(`User report resolved for: ${reportData.reportedUserId} in room ${reportData.roomId}`);
    }
